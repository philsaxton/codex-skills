...
Use the printed absolute scratch path for explicit temporary files and process-scoped `TMPDIR`, `TMP`, or `TEMP` settings where supported; verify actual tool output placement instead of assuming every tool honors those settings. Some workflows, such as tests that create a new workspace, require temporary directories outside any enclosing Git repository. Use an allowed external temporary location for those cases and record the reason; do not weaken repository-boundary checks or treat the alternate-location test as proof of garage-local access. Keep deliverables, reports and migration recovery evidence outside disposable scratch.

Completion is the owning caller's declaration that writers stopped and contents are disposable; the helper cannot detect every active process or infer retention policy. Cleanup previews by default and permanently removes only the named completed task on `--apply`. It refuses unmarked/active directories, path arguments, repositories, symlinks, hard links, special files and nested filesystem boundaries.

Directory-descriptor operations avoid following substituted symlinks and refuse detected changes. They do not isolate concurrent filesystem activity: an already-open directory can still be modified through its descriptor after another process relocates it. Stop writers and do not relocate scratch during cleanup. A mid-cleanup failure can leave partial removal and is not automatically rolled back. Use the existing authorization for the declared disposable task, not a new blanket deletion grant.
