...
Write lasting workspace plans, decisions and migration contracts under `docs/` by default. Keep application-specific docs in the application's index, application reports in `artifacts/`, workspace backups and recovery evidence in `.recovery/`, and disposable work in scratch. For existing garages, reconcile the selected documentation and tracking rules without moving unrelated material.
