...
For a migration, inspect first and obtain approval of the concrete destination layout before filesystem or index changes. Reuse an already approved specific plan; general organization requests do not approve an unseen layout. Follow the [refactoring guidance](references/refactoring.md) for layout choices, the migration map, and material departures from approved scope.
...
| Workspace documentation | Lasting workspace organization plans, migration contracts and workspace decisions. Default to tracked garage documentation under `docs/`, with the authoritative migration record at `docs/workspace-migration.md`; application-specific documentation remains with the application. |
| Retained application outputs | Application-generated reports, output documents and exchanges between applications, typically in ignored `artifacts/`. Retention does not imply Git tracking. |
| Workspace recovery | Raw backups, preservation manifests and recovery evidence in ignored `.recovery/`, with migration evidence under `.recovery/workspace-migration/`. These support the tracked migration record; they are not its authoritative home. |
...
| Workspace support | Development-agent instructions, selected skills, shared tools, and workspace conventions. Default vendored skill dependencies to `.agents/vendors/<dependency>/` and activation links to `.agents/skills/`. Version-control the portable support; keep secrets and machine-local state out. |

Classify by purpose, not filename or whether an agent generated it. A workspace migration contract governs the workspace and belongs in its tracked documentation; it is not a runtime report. A supported MCP endpoint can be product code; a local agent adapter can be garage tooling. An intentional test fixture is not disposable merely because it was generated. Keep workspace recovery data in `.recovery/` by default, separate from application outputs in `artifacts/` and disposable scratch; keep it ignored unless deliberately selected for tracking and use access-appropriate storage for sensitive evidence; reference that evidence from the contract without copying sensitive contents into it. Preserve the user's chosen ownership when it differs from these defaults.
...
4. **Set up only needed support.** Install or configure skills and tools required by the request using the chosen installation mechanism and existing authorization. For vendored skill dependencies, use `.agents/vendors/<dependency>/` (for example `.agents/vendors/codex-skills/`), with selected activation links under `.agents/skills/`. The `vendors/` container belongs inside `.agents/`, not at the garage root; keep each dependency pinned separately when using submodules. For example, `.agents/skills/workspace-setup` links to `../vendors/codex-skills/workspace-setup`. Create these paths only when installing needed support, and preserve an explicitly chosen alternative installation layout. Separate portable configuration from local credentials and machine-specific state. When the application itself supplies skills, distinguish its editable source from the selected installed version; retain deliberate pins or copies rather than silently linking active instructions to a changing checkout. Report which version is active. Do not silently relocate protected control files or broaden permissions to make setup succeed.
...
- Verify application output, recovery, scratch and worktree paths are ignored. Check effective access and temporary output placement using the selected workflow; setup does not require installing or exercising a particular cleanup helper. System temporary directories may already be permitted; garage-local storage is an organization/default-location choice, not a permission override.
