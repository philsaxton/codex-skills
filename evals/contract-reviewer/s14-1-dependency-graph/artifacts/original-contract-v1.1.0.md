...
- **Revision-author baseline:** checkpoint `0ef6a7a6d414cf1d6e43e8ea52700943a39a0351`
...
## Revision history and B-01 disposition

Version 1.0.0, SHA-256 `021da09fc937958c84db8334662a6bb869d52f643d8e5476b26dcb87c2d5780d`, was checkpointed at `e707602f75391eeed5605c0fd25de39bac9400c4` and received `Changes required / Not approved` in `docs/reviews/FR-07-contract-review.md`, SHA-256 `f8d2fd1d5b588ea2c0de94ad0527b60bb938ce3818eb4886fd1eaaf285e600d`, checkpointed at `0ef6a7a6d414cf1d6e43e8ea52700943a39a0351`. That complete adverse decision is preserved and authorizes no implementation.

**B-01 disposition in v1.1.0:** resolved by placing the completed S1.2 slice directly on every new child that consumes its approved security artifacts or executable obligations: S14.1 depends on S1.2; S14.2 depends on S1.2 and S14.1; S14.3 depends on S1.2 and S14.2. No child names E1 because epic state is only a coordination summary, and no child separately names S1.1 because S1.2 cannot be complete without S1.1 and is the smallest concrete completed security-foundation gate. S14.2 and S14.3 nevertheless name S1.2 directly because each consumes S1.2 artifacts or gate obligations; their dependency is not left implicit through another E14 child. The exact table, sequence, parser checks, criteria, completion preconditions, risk language, and handoff boundary below are updated consistently. All v1.0.0 architecture, placement, scope, proposal, authority, and preservation boundaries remain unchanged except these dependency and necessary cross-reference edits.
...
| FR-07 v1.0.0 contract review | SHA-256 `f8d2fd1d5b588ea2c0de94ad0527b60bb938ce3818eb4886fd1eaaf285e600d1` at checkpoint `0ef6a7a6d414cf1d6e43e8ea52700943a39a0351`; preserved `Changes required / Not approved` and B-01 dependency finding |
...
| S14.1 | Reconcile the S2.1 security boundary and dependent architecture | Slice | P0 | In progress | Done | S1.2 | None |
| S14.2 | Extract the reusable application security kernel | Slice | P0 | Planned | Ready | S1.2 and S14.1 | None |
| S14.3 | Implement authoritative hosted security persistence | Slice | P0 | Planned | Planned | S1.2 and S14.2 | None |

E14 purpose: establish one reusable server security kernel and one authoritative hosted persistence boundary before profile lifecycle or hosted endpoint work continues. E14 acceptance requires S14.1, S14.2, and S14.3 `Done` with their independent evidence; its verification cites each owning contract/evidence/final review. E14's `Dependencies: E1` is a coordination summary only and is not inherited. Each child carries its actual concrete S1.2 prerequisite as specified above. E14 remains `Planned` after S14.1 because two required children remain outstanding.

S14.1 purpose, acceptance, verification, and notes MUST faithfully describe this documentation-only contract, exact approved identities, preservation, no proposal amendment, no owner decision, serialization, and downstream non-authorization. It directly depends on S1.2 because this correction consumes the completed S1.1/S1.2 security foundation; S1.2 already directly depends on S1.1, so a separate S1.1 edge would be redundant, while E1 is not an executable inherited gate. Its implementation state is `In progress`; only the final-review-approved completion patch may change it to `Done`.

S14.2 purpose: extract a protocol-neutral kernel from the approved S1.1/S1.2 behavior and useful preserved S2.1 work, including authenticated context consumption, tenant/actor binding, authorization primitives, audit intent/provenance construction, idempotency/concurrency, transaction interface, and defense-in-depth integrity checks. It directly depends on both S14.1 for the corrected architecture and S1.2 because it consumes the exact approved S1.2 contract/security artifacts and must run their regressions; those consumed prerequisites are not hidden behind S14.1. Its acceptance/verification MUST explicitly disclaim in-memory tamper resistance and require failing-first application tests plus S1.1/S1.2 regressions. It performs no database migration, endpoint build, profile completion, merge of the preserved branch, or external action.

S14.3 purpose: implement the authoritative hosted Supabase/Postgres security persistence boundary for the kernel, including restricted credentials, permissions/RLS as applicable, atomic transactions, append-only enforcement, constraints/indexes, and adverse integration tests. It directly depends on S14.2 for the kernel transaction/persistence interface and on S1.2 because it consumes and must map the applicable S1.2 write-runtime, audit-integrity, tenant-isolation, and adverse-test obligations. It does not provision production infrastructure, build MCP/REST endpoints, complete profile behavior, or perform deployment. It remains `Planned` until both direct dependencies are `Done`.
...
S1.1 -> S1.2
          ...
          -> S14.2 reusable kernel extraction (also depends on S14.1)
          -> S14.3 authoritative hosted persistence (also depends on S14.2)
       -> S2.1 rebased profile lifecycle -> S2.2 workflow resolution/brief compilation
       -> S3.1 hosted service and MCP/REST peer adapters
```

The direct child dependency sets are exact: S14.1 = `{S1.2}`, S14.2 = `{S1.2, S14.1}`, and S14.3 = `{S1.2, S14.2}`. E1 appears only on E14 as a coordination summary; S1.1 is reached through completed S1.2 and is not redundantly repeated on the new children. After S14.3, S2.1 and S3.1 are independent only if their approved contracts and exact file boundaries do not overlap. Any shared kernel/persistence interface, migration, service configuration, or unresolved dependency requires serialization. S2.2 cannot begin until the rebased S2.1 is `Done`.
...
- **AC-07-03 — Kernel boundary:** Server-derived authenticated context, tenant/actor binding, reusable authorization, audit construction/provenance, idempotency/concurrency, transaction interface, and applicable integrity checks have one protocol-neutral owner and preserve S1.1/S1.2; S14.1/S14.2/S14.3 each directly names S1.2 where it consumes that foundation.
...
- **AC-07-08 — Exact backlog graph:** E14/S14.1–S14.3 and affected E2/S2.1/S2.2/E3/S3.1 fields equal this contract; the three new child dependency sets are exactly `{S1.2}`, `{S1.2, S14.1}`, and `{S1.2, S14.2}` respectively; E1 remains epic-summary-only, S1.1 is reached through S1.2, every reference resolves, no cycle or hidden gate exists, and no unearned item becomes active or complete.
...
- **AC-07-12 — Completion projection:** With S1.2 already `Done`, the exact completion payload changes only permitted completion fields for E14/S14.1/S14.2, moves S14.1 to `Done` and S14.2 to `Ready` only after both of S14.2's direct dependencies are complete, leaves E14 and S14.3 `Planned`, and does not alter or complete S2.1/S2.2/S3.1.
...
3. a deterministic backlog parser proving unique IDs, controlled fields/statuses, the exact new-child dependency sets S14.1 = `{S1.2}`, S14.2 = `{S1.2, S14.1}`, and S14.3 = `{S1.2, S14.2}`, E1's epic-summary-only treatment, S1.2 -> S1.1 reachability, exact blockers, all references resolved, no self-dependency/cycle, exact E14 children, and exact S2.1/S2.2/S3.1 gates;
...
3. S14.2 existing `Status: Planned` to `Status: Ready` only if both direct dependencies S1.2 and S14.1 are `Done`, and its existing `Verification`/`Notes` only enough to record those satisfied gates and that S14.2 itself is unstarted; and
...
- **Risk — hidden or circular dependency:** Mitigated by exact direct sets S14.1 = `{S1.2}`, S14.2 = `{S1.2, S14.1}`, and S14.3 = `{S1.2, S14.2}`, followed by independent S2.1/S3.1 branches. E1 is not inherited and S1.1 is reached through S1.2. Stop on a missing, unresolved, redundant-without-consumption-basis, or cyclic dependency output.
...
The revision-author checkpoint contains only this revised contract. The exact full checkpoint SHA and final contract SHA-256 are supplied in the handoff after commit to avoid self-reference. Stage-relative inventory relative to review checkpoint `0ef6a7a6d414cf1d6e43e8ea52700943a39a0351` must be exactly `M docs/contracts/FR-07-s2-1-security-boundary-architecture-correction.md`; stage-relative state must be clean after commit. The preserved adverse review and all other pre-existing/out-of-scope paths, including the current local-main governance state and preserved S2.1 branch/checkpoint, remain outside the commit and are neither included, overwritten, reverted, nor deleted.
